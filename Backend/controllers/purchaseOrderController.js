...
export const createPurchaseOrder = async (req, res) => {
  try {
    const { po_number, pr_id, vendor_id, issue_date, expected_delivery_date, payment_terms, subtotal, tax, total_amount, notes } = req.body;

    // Validate required fields
    if (!po_number || !pr_id || !vendor_id || !issue_date || !subtotal) {
...
export const getPurchaseOrderById = async (req, res) => {
  try {
    ...
    const purchaseOrder = await PurchaseOrder.findById(id)
      .populate('pr_id', 'pr_number department')
      .populate('vendor_id', 'vendor_name email')
      .populate('issuer_id', 'full_name email');
